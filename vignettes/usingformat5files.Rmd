...
has_vcfppR <- requireNamespace("vcfppR", quietly = TRUE)
```
...
Reading Format 5 files requires the
[vcfppR](https://CRAN.R-project.org/package=vcfppR) package. Install it with
...
```{r convert, eval = has_vcfppR, echo = T, message = F, warning = F}
...
```{r load_info, eval = has_vcfppR, echo = T, message = F, warning = F}
...
```{r samples, eval = has_vcfppR, echo = T}
...
```{r indices, eval = has_vcfppR, echo = T}
...
```{r snp_by_index, eval = has_vcfppR, echo = T, message = F, warning = F}
...
```{r snp_by_id, eval = has_vcfppR, echo = T, message = F, warning = F}
snp_id <- "1:12000:T:C"
...
```{r apply, eval = has_vcfppR, echo = T, message = F, warning = F}
...
```{r cleanup, include = FALSE, eval = has_vcfppR}
